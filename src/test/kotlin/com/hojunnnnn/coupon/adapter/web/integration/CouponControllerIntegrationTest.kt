...
class CouponControllerIntegrationTest @Autowired constructor(
    ...
    inner class `쿠폰 생성` {
        ...
        @CsvSource(
            "'a  b', 10", "'   ', 10",
            "TEST_COUPON, -1", "TEST_COUPON, 0"
        )
        fun `유효하지 않은 파라미터로 요청시 에러를 반환한다`(name: String, quantity: Int) {
    ...
    inner class `쿠폰 발급` {
        ...
        fun `성공`() {
            ...
            val resultActions = mockMvc.post("/api/v1/coupons/${coupon.id}/issue") {
